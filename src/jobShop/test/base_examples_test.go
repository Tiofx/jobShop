...
func ExampleTimeOfSchedule() {
	solution := tabuSearch.Solve(ComplexTestCase().Jobs, 100, 16, 10)
...
func ExampleSolution1() {
	solution := tabuSearch.Solve(ComplexTestCase().Jobs, 100, 16, 10)
	fmt.Println("Order of job in which schedule has to be accomplish:")
	fmt.Println(solution.JobOrder)

	// Output:
	// Order of job in which schedule has to be accomplish to achieve calculated time
...
//----------------------------------
func ExampleTest() {
	res := TabuSearch(taskWaveByMachineGreed.OptimalPermutationLimit, 1000, 32, 55, TestCaseNumber(6))[0]
	fmt.Println(res)
}
...
//---------------------------
func ExampleTests() {
	res := TabuSearch(taskWaveByMachineGreed.OptimalPermutationLimit, 100, 16, 32, AllTestsCases()[:5]...)
	fmt.Println(res)
}
